...
public class Woodcutting {
	...
	private int checkWcEmote(int axeType) {
		if (axeType == 1359)
			return 867;
		else if (axeType == 1349)
			return 877;
		else if (axeType == 6739)
			return 2846;
		return -1;
	...
	public void startWoodcutting(int logType, int levelReq, int exp) {
		if (goodAxe() > 0) {
			...
			if (c.playerLevel[c.playerWoodcutting] >= levelReq) {
				...
				c.startAnimation(checkWcEmote(goodAxe()));
			} else {
				c.getPA().resetVariables();
				c.startAnimation(65535);
				c.sendMessage("You need a woodcutting level of " + levelReq
						+ " to cut this tree.");
			}
		} else {
			c.startAnimation(65535);
	...
	public void cutWood() {
		if (c.getItems().addItem(logType, 1)) {
			...
			c.getPA().addSkillXP(exp * Config.WOODCUTTING_EXPERIENCE,
					c.playerWoodcutting);
