...
public class CombatAssistant{
	...
	public void applyNpcMeleeDamage(final int i, final int damageMask) {
		...
		final boolean fullVeracsEffect = c.getPA().fullVeracs() && Misc.random(3) == 1;
	...
	public void attackPlayer(final int i) {
		final Client c2 = (Client)PlayerHandler.players[i]; //the player we are attacking
		if (PlayerHandler.players[i] != null) {
			...
			final boolean sameSpot = c.absX == PlayerHandler.players[i].getX() && c.absY == PlayerHandler.players[i].getY();
	...
	public boolean checkMultiBarrageReqs(final int i) {
		...
		if(Config.COMBAT_LEVEL_DIFFERENCE) {
			final int combatDif1 = c.getCombat().getCombatDifference(c.combatLevel, PlayerHandler.players[i].combatLevel);
	...
	public boolean checkReqs() {
		...
		if(Config.COMBAT_LEVEL_DIFFERENCE) {
			final int combatDif1 = c.getCombat().getCombatDifference(c.combatLevel, PlayerHandler.players[c.playerIndex].combatLevel);
	...
	public int getAttackDelay(final String s) {
		...
		int standardSpeed = 0;
		standardSpeed += 4.0;
		final String [] standard = {"Dagger","scimitar","shortbow","Karil's c'bow",
		...
		final String [] slow = {"Longsword","mace","hatchet","spear","pickaxe","TzHaar-Ket-Em","Torag's_hammers","Guthan's_warspear",
		...
		final String [] verySlow = {"Battleaxe","warhammer","Godsword","Barrelchest anchor","Ahrim's_staff","Toktz-Mej-Tal","chaotic maul",
		...
		final String [] extremelySlow = {"2h sword","halberd","Granite maul","Balmung","TzHaar-Ket-Om","Dharok's greataxe"};
		...
		for (final int[] slayerReq : slayerReqs) {
			if (slayerReq[0] == NPCHandler.npcs[i].npcType) {
				if (slayerReq[1] > c.playerLevel[c.playerSlayer]) {
					c.sendMessage("You need a slayer level of " + slayerReq[1] + " to harm this NPC.");
					return false;
				}
			}
		}
			...
			if (c.goodDistance(c.getX(), c.getY(), o.getX(), o.getY(), c.getCombat().getRequiredDistance())) {
				if (c.getCombat().checkReqs()) {
					if (c.getCombat().checkSpecAmount(4153)) {
						final boolean hit = Misc.random(c.getCombat().calculateMeleeAttack()) > Misc.random(o.getCombat().calculateMeleeDefence());
						int damage = 0;
						if (hit) {
	...
	public int rangeMaxHit() {
		...
		final int d = getRangeStr(c.usingBow ? c.lastArrowUsed : c.lastWeaponUsed);
		double b = 1.00;
		if (c.prayerActive[3]) {
			b *= 1.05;
		} else if (c.prayerActive[11]) {
