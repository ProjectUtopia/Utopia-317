package com.rakeyjake.server.model.shops;
...
public class ShopAssistant {
	...
	public void buyFromShopPrice(int removeId, int removeSlot) {
		...
		ShopValue *= 1.15;
		String ShopAdd = "";
		if (c.myShopId == 17 || c.myShopId == 18) {
	...
	public boolean buyItem(int itemID, int fromSlot, int amount) {
		...
		if (amount > 0) {
			...
			int Slot = 0;
			int Slot1 = 0;// Tokkul
			if (c.myShopId == 17 || c.myShopId == 18 || c.myShopId == 42 || c.myShopId == 50
					|| c.myShopId == 60) {
			...
			for (int i = amount; i > 0; i--) {
				...
				if (c.myShopId != 29 || c.myShopId != 30 || c.myShopId != 31 || c.myShopId == 14) {
	...
	public void handleOtherShop(int itemID) {
		...
		} else if (c.myShopId == 42) { //pkpointshop#1
