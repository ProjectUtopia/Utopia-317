...
public class PlayerSave {
			...
			if (spot > -1) {
				...
				case 2:
					if (token.equals("character-height")) {
						...
						p.pkPoints = Integer.parseInt(token2);
					} else if (token.equals("totalKills")) {
						p.kills = Integer.parseInt(token2);
					} else if (token.equals("totalDeaths")) {
						...
						p.jailed = Boolean.parseBoolean(token2);
	...
	public static boolean saveGame(Client p) {
		...
		try {
			...
			characterfile.newLine();
			characterfile.write("totalDeaths = ", 0, 14);
			...
			characterfile.write(Boolean.toString(p.jailed), 0, Boolean
					.toString(p.jailed).length());
			characterfile.newLine();
