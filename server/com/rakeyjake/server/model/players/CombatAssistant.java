...
public class CombatAssistant {
	...
	public CombatAssistant(final Client Client) {
		c = Client;
	}

	/** Prayer **/
	@SuppressWarnings("unused")
	public void activatePrayer(final int i) {
		if (c.duelRule[7]) {
	...
	public void activateSpecial(final int weapon, final int i) {
	...
	public void appendMultiBarrage(final int playerId, final boolean splashed) {
	...
	public void appendVengeance(final int otherPlayer, int damage) {
		if (damage <= 0) {
			return;
		}
	...
	public void applyNpcMeleeDamage(final int i, final int damageMask) {
		int damage = Misc.random(calculateMeleeMaxHit());
		final boolean fullVeracsEffect = c.getPA().fullVeracs()
				&& Misc.random(3) == 1;
	...
	public void applyPlayerMeleeDamage(final int i, final int damageMask) {
		final Client o = (Client) PlayerHandler.players[i];
		if (o == null) {
			return;
		}
	...
	public void applyRecoil(final int damage, final int i) {
		if (damage > 0
	...
	public void applySmite(final int index, final int damage) {
	...
	public boolean armaNpc(final int i) {
	 ...
	 */
	@SuppressWarnings("unused")
	public void attackNpc(final int i) {
		if (NPCHandler.npcs[i] != null) {
	 ...
	 **/
	@SuppressWarnings("unused")
	public void attackPlayer(final int i) {
		final Client c2 = (Client) PlayerHandler.players[i]; // the player we
																// are attacking
		if (PlayerHandler.players[i] != null) {
			...
			final boolean sameSpot = c.absX == PlayerHandler.players[i].getX()
			...
			// if (Player.heightLevel != Player.heightLevel) {
			// resetPlayerAttack();
			// return;
			// }
			...
			if (c.attackTimer <= 0) {
				...
				if (c.getX() != c2.getX() && c.getY() != c2.getY()
						&& !usingOtherRangeWeapons && !usingHally()
						&& !usingBow && !c.usingMagic) {
	...
	public int calculateMeleeDefence() {
		...
		final int i = c.playerBonus[bestMeleeDef()];
		if (c.prayerActive[0]) {
	...
	public int calculateMeleeMaxHit() {
		double maxHit = 0;
		final int strBonus = c.playerBonus[10];
		int strength = c.playerLevel[2];
		final int lvlForXP = c.getLevelForXP(c.playerXP[2]);
		if (c.prayerActive[1]) {
	...
	@SuppressWarnings("unused")
	public boolean checkMagicReqs(final int spell) {
	...
	public boolean checkMultiBarrageReqs(final int i) {
		if (PlayerHandler.players[i] == null) {
			return false;
		}
		if (i == c.playerId) {
		...
		if (Config.COMBAT_LEVEL_DIFFERENCE) {
			final int combatDif1 = c.getCombat().getCombatDifference(
	...
	public boolean checkSpecAmount(final int weapon) {
		switch (weapon) {
		case 1249:
		case 1215:
		case 1231:
	...
	public void delayedHit(final int i) { // npc hit delay
		if (NPCHandler.npcs[i] != null) {
			...
			if (!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				for (final int noArrowId : c.NO_ARROW_DROP) {
	...
	public void freezePlayer(final int i) {
	 ...
	 **/
	public int getAttackDelay(final String s) {
		int fastSpeed = 0;
		fastSpeed += 3.0;
		final String[] fast = { "dart", "knife", "knife(p++)" };
		for (final String element : fast) {
		...
		int standardSpeed = 0;
		standardSpeed += 4.0;
		final String[] standard = { "Dagger", "scimitar", "shortbow",
				"Karil's c'bow", "Toktz-Xil-Ul", "claws", "Zamorakian Spear",
				"Saradomin Sword", "Toktz-Xil-Ak", "Toktz-Xil-Ek",
				"Abyssal whip", "Slayer's staff", "Ancient staff", "rapier" };
		for (final String element : standard) {
			if (s.contains(element)) {
				return standardSpeed;
			}
		}
		final String[] slow = { "Longsword", "mace", "hatchet", "spear",
				"pickaxe", "TzHaar-Ket-Em", "Torag's_hammers",
				"Guthan's_warspear", "Verac's_flail", "Staff of light",
				"Throwing axe", "Crystal bow", "Ogre composite bow", "Seercull" };
		final int slowSpeed = 5;
		for (final String element : slow) {
			if (s.contains(element)) {
				return slowSpeed;
			}
		}

		final String[] verySlow = { "Battleaxe", "warhammer", "Godsword",
				"Barrelchest anchor", "Ahrim's_staff", "Toktz-Mej-Tal",
				"chaotic maul", "Gravite_2h_sword", "Longbow", "God bow",
				"Javelin", "crossbow", "Hand cannon" };
		int verySlowSpeed = 0;
		verySlowSpeed += 6.0;
		for (final String element : verySlow) {
			if (s.contains(element)) {
				return verySlowSpeed;
			}
		}
		final String[] extremelySlow = { "2h sword", "halberd", "Granite maul",
				"Balmung", "TzHaar-Ket-Om", "Dharok's greataxe" };
		int extremelySlowSpeed = 0;
		extremelySlowSpeed += 7.0;
		for (final String element : extremelySlow) {
			if (s.contains(element)) {
	...
	public int getBonusAttack(final int i) {
		switch (NPCHandler.npcs[i].npcType) {
		case 2883:
	...
	public int getCombatDifference(final int combat1, final int combat2) {
		if (combat1 > combat2) {
	 ...
	 **/
	public int getHitDelay(final String weaponName) {
		if (c.usingMagic) {
			switch (c.MAGIC_SPELLS[c.spellId][0]) {
			case 12891:
				return 4;
	...
	public int getKillerId(final int playerId) {
		int oldDamage = 0;
		int killerId = 0;
	...
	public void getPlayerAnimIndex(final String weaponName) {
	...
	public int getRangeStr(final int i) {
		int str = 0;
		final int[][] data = { { 877, 10 }, { 9140, 46 }, { 9145, 36 },
				{ 9141, 64 }, { 9142, 82 }, { 9143, 100 }, { 9144, 115 },
				{ 9236, 14 }, { 9237, 30 }, { 9238, 48 }, { 9239, 66 },
				{ 9240, 83 }, { 9241, 85 }, { 9242, 103 }, { 9243, 105 },
				{ 9244, 117 }, { 9245, 120 }, { 882, 7 }, { 884, 10 },
	...
	public int getWepAnim(final String weaponName) {
	...
	public boolean goodSlayer(final int i) {
		for (final int[] slayerReq : slayerReqs) {
			if (slayerReq[0] == NPCHandler.npcs[i].npcType) {
				if (slayerReq[1] > c.playerLevel[c.playerSlayer]) {
					c.sendMessage("You need a slayer level of " + slayerReq[1]
							+ " to harm this NPC.");
					return false;
				}
			}
	...
	public void handleGmaulPlayer() {
		if (c.playerIndex > 0) {
			...
			final int x = NPCHandler.npcs[c.npcIndex].absX;
			final int y = NPCHandler.npcs[c.npcIndex].absY;
	...
	public void multiSpellEffect(final int playerId, final int damage) {
		switch (c.MAGIC_SPELLS[c.oldSpellId][0]) {
		case 13011:
		case 13023:
			if (System.currentTimeMillis()
	...
	public int rangeMaxHit() {
		final int a = c.playerLevel[4];
		final int d = getRangeStr(c.usingBow ? c.lastArrowUsed
				: c.lastWeaponUsed);
		double b = 1.00;
		if (c.prayerActive[3]) {
			b *= 1.05;
		...
		final double max = (1.3 + e / 10 + d / 80 + e * d / 640) * darkbow;
		return (int) max;
	}
	...
	public boolean wearingStaff(final int runeId) {
		final int wep = c.playerEquipment[c.playerWeapon];
		switch (runeId) {
		case 554:
			if (wep == 1387) {
				return true;
